const crypto = require('crypto');
const config = require('./config.json');

const UDP_KEY = crypto.createHash('md5').update(config.udpKey, 'utf8').digest();

/**
* Low-level class for encrypting and decrypting payloads.
* @property {String} key the local key used of this cipher
* @property {String} version the protocol version
*/
class TuyaCipher {
  /**
   *
   * @param {Object} options
   * @param {String} options.key localKey of cipher
   * @param {Number} options.version protocol version
   * @example
   * const cipher = new TuyaCipher({key: 'xxxxxxxxxxxxxxxx', version: 3.1})
   */
  constructor(options) {
    this.key = options.key;
    this.version = options.version.toString();
  }

  /**
  * Encrypts data.
  * @param {Object} options
  * @param {String|Buffer} options.data data to encrypt
  * @param {false=} [options.base64] `true` to return result in Base64
  * @example
  * TuyaCipher.encrypt({data: 'hello world'})
  * @returns {String|Buffer} returns Buffer unless options.base64 is true
  */
  encrypt(options) {
    const cipher = crypto.createCipheriv('aes-128-ecb', this.key, '');

    let encrypted = typeof options.data === 'string' ?
      cipher.update(options.data, 'utf8', 'base64') :
      // Help typescript deal with this - the type definitions for
      // cipher.update are rather strict.
      // These two cases are actually identical, because, according to
      // eslint-disable-next-line max-len
      // https://nodejs.org/api/crypto.html#crypto_cipher_update_data_inputencoding_outputencoding
      // the enconding argument is ignored when the data is a buffer.
      cipher.update(options.data, undefined, 'base64');

    encrypted += cipher.final('base64');

    // Default base64 enable
    if (options.base64 === false) {
      return Buffer.from(encrypted, 'base64');
    }

    return encrypted;
  }

  /**
  * Decrypts data.
  * @param {String|Buffer} data to decrypt
  * @returns {Object|String}
  * returns object if data is JSON, else returns string
  */
  decrypt(data) {
    // Incoming data format
    /** @type {undefined | "base64"} */
    let format;

    if (data.indexOf(this.version) === 0) {
      if (this.version === '3.3') {
        // Remove 3.3 header
        data = data.slice(15);
      } else {
        // Data has version number and is encoded in base64

        // NOTE: This is likely a bug - if data is a Buffer, then value of
        // format will be ignored as per
        // eslint-disable-next-line max-len
        // https://nodejs.org/api/crypto.html#crypto_cipher_update_data_inputencoding_outputencoding
        // Remove prefix of version number and MD5 hash
        data = data.slice(19).toString();
        // Decode incoming data as base64
        format = 'base64';
      }
    }

    // Decrypt data
    let result;
    try {
      const decipher = crypto.createDecipheriv('aes-128-ecb', this.key, '');
      // @ts-ignore This should be fixed so that the ignore is not needed; See note above
      result = decipher.update(data, format, 'utf8');
      result += decipher.final('utf8');
    } catch (_) {
      // Try the universal key, in case it's a new UDP message format
      try {
        const decipher = crypto.createDecipheriv('aes-128-ecb', UDP_KEY, '');
        // This should be fixed so that the ignore is not needed; See note above
        // @ts-ignore
        result = decipher.update(data, format, 'utf8');
        result += decipher.final('utf8');
      } catch (_) {
        throw new Error('Decrypt failed');
      }
    }

    // Try to parse data as JSON,
    // otherwise return as string.
    try {
      return JSON.parse(result);
    } catch (_) {
      return result;
    }
  }

  /**
  * Calculates a MD5 hash.
  * @param {String} data to hash
  * @returns {String} characters 8 through 16 of hash of data
  */
  md5(data) {
    const md5hash = crypto.createHash('md5').update(data, 'utf8').digest('hex');
    return md5hash.slice(8, 24);
  }
}
module.exports = TuyaCipher;
