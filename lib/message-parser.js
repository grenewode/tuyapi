...
class MessageParser {
   ...
   * @param {Array.<Packet>} packets that have been parsed
   * @returns {Array.<Packet>} array of parsed packets
   */
  parseRecursive(buffer, packets) {
    const result = this.parsePacket(buffer);

    // As far as we know, result.payload will always be a buffer when returned
    // from this.parsePacket. However, it is hard to tell this to the
    // type checker in a nice way and this will protect us in case
    // this.parsePacket is ever updated to return something else
    const payload = result.payload instanceof Buffer ?
      this.getPayload(result.payload) :
      result.payload;
   ...
   * Tuya-protocol-compliant packet.
   * @param {Object} options
  ...
  encode(options) {
    ...
    // technically, version is always a string because that is
    // how it is set in the constructor
    // @ts-ignore
    if (this.version === 3.3 || this.version === '3.3') {
      // Encrypt data
      ...
      if (options.commandByte !== CommandType.DP_QUERY) {
        ...
        // We know that payload must be a Buffer after it is returned
        // by this.cipher.encrypt
        // @ts-ignore
    ...
    // We know that payload must be a Buffer after it is returned by this.cipher.encrypt
    // @ts-ignore
